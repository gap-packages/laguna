...
#W                                                            Csaba Schneider
##
#H  $Id: laguna.gi,v 1.11 2003/07/22 19:47:16 alexk Exp $
##
#############################################################################
...
InstallImmediateMethod( IsFModularGroupAlgebra,
    IsGroupAlgebra, 0,
    R -> IsFinite(UnderlyingMagma(R)) and Characteristic(LeftActingDomain(R)) 
<> 0 and 
...
InstallImmediateMethod( IsPModularGroupAlgebra,
    ...
    R -> IsPGroup(UnderlyingMagma(R)) and Characteristic(LeftActingDomain(R)) 
<> 0 and 
...
InstallMethod( Involution,
    "LAGUNA: for an element of a group ring and a mapping of order 2 of a group onto itself",
    true,
        ...
        for c in cc do
                ...
                if IsomorphismGroups(G,H) <> fail then
                        ...
                        if Dimension(FH)=Size(G) then
                                ...
                                Info(LAGInfo, 3, 
				    "LAGInfo: H is a group basis");
                        else
                                Info(LAGInfo, 3, 
	                            "LAGInfo: H linearly dependent");
                        fi;
                else
...
InstallMethod( DimensionBasis,
    ...
    function( g )
        ...
        for i in [ 1 .. Length(j)-1 ] do        
            ...
            gens := Filtered( GeneratorsOfGroup( Image( n )), 
	                                        x -> not IsOne(x) );
